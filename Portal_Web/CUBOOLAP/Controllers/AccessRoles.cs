using CUBOOLAP.Data;
using CUBOOLAP.Models;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using System.Threading.Tasks;

namespace CUBOOLAP.Controllers
{
    [Authorize] // Aplica autorización a todo el controlador
    public class AccessRolesController : Controller
    {
        private readonly ApplicationDbContext _context;

        public AccessRolesController(ApplicationDbContext context)
        {
            _context = context;
        }

        // GET: AccessRoles
        public ActionResult Index()
        {
            return View();
        }

        // GET: AccessRoles/Details/5
        public ActionResult Details(int id)
        {
            return View();
        }

        // GET: AccessRoles/Create
        public ActionResult Create()
        {
            return View();
        }

        // POST: AccessRoles/Create
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create(IFormCollection collection)
        {
            try
            {
                return RedirectToAction(nameof(Index));
            }
            catch
            {
                return View();
            }
        }

        // GET: AccessRoles/Edit/5
        public ActionResult Edit(int id)
        {
            return View();
        }

        // POST: AccessRoles/Edit/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(int id, IFormCollection collection)
        {
            try
            {
                return RedirectToAction(nameof(Index));
            }
            catch
            {
                return View();
            }
        }

        // GET: AccessRoles/Delete/5
        public ActionResult Delete(int id)
        {
            return View();
        }

        // POST: AccessRoles/Delete/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(int id, IFormCollection collection)
        {
            try
            {
                return RedirectToAction(nameof(Index));
            }
            catch
            {
                return View();
            }
        }

        // Controlador específico para roles de administrador
        [Authorize(Roles = "Administrador")]
        public class AdminController : Controller
        {
            public IActionResult Index()
            {
                return View();
            }
        }



        // Método POST para actualizar los datos
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditData(int id, DataModel dataModel)
        {
            if (id != dataModel.Id)
            {
                return BadRequest();
            }

            if (ModelState.IsValid)
            {
                _context.Update(dataModel);
                await _context.SaveChangesAsync();
                return RedirectToAction(nameof(Index));
            }
            return View(dataModel);
        }
    }
}
